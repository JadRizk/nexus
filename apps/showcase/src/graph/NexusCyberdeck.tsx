...
export default function NexusCyberdeck() {
  ...
  const [nodeOn, setNodeOn] = useState(() => Object.fromEntries(NODE_CATEGORY_IDS.map((k) => [k, true])));
  const [linkOn, setLinkOn] = useState(() => Object.fromEntries(LINK_CATEGORY_IDS.map((k) => [k, true])));

  const [cfg, setCfg] = useState(DEFAULT_CFG);
  const set = useCallback(<K extends keyof typeof DEFAULT_CFG>(k: K, v: (typeof DEFAULT_CFG)[K]) => setCfg((p) => ({ ...p, [k]: v })), []);
  ...
  const physics: Partial<PhysicsConfig> = {
    repulsion: cfg.repulsion, linkDistance: cfg.linkDistance, cursorForce: cfg.cursorForce, settle: cfg.settle,
  };
  const optics: Partial<OpticsConfig> = {
    glow: cfg.glow, trails: cfg.trails, edgeOpacity: cfg.edgeOpacity, edgeWidth: cfg.edgeWidth, flowSpeed: cfg.flowSpeed,
    scan: cfg.scan, aberr: cfg.aberr, curve: cfg.curve, grain: cfg.grain, bloom: cfg.bloom, glitch: cfg.glitch,
  };
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") { setSelected(null); setIsolate(null); }
    };
  ...
  return (
    <div data-nx-theme="hud" style={{
      ...
      {/* -------------------------------------------------- left: console+legend */}
      <div style={{
        position: "absolute", top: "var(--nx-space-5)", left: "var(--nx-space-5)", width: 248,
        display: "flex", flexDirection: "column", gap: "var(--nx-space-3)",
        maxHeight: "calc(100% - var(--nx-space-6))", overflowY: "auto", overflowX: "hidden", scrollbarWidth: "none",
      }}>
        <Panel style={{ flexShrink: 0 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) minmax(0, 1fr)", gap: "1px var(--nx-space-2)" }}>
          ...
          {tab === "crt" ? (
            ...
            <>
              ...
              <Slider label="corpus size" value={total} min={60} max={600} step={20}
                onChange={(v) => { setSelected(null); setIsolate(null); setTotal(v); }} />
            </>
          )}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) minmax(0, 1fr)", gap: "var(--nx-space-2)", marginTop: "var(--nx-space-3)" }}>
            <Button style={{ minWidth: 0 }} onClick={() => controllerRef.current?.fit()}>Fit</Button>
            <Button style={{ minWidth: 0 }} onClick={() => controllerRef.current?.reheat()}>Reheat</Button>
            <Button style={{ minWidth: 0 }} onClick={() => setRunning((r) => !r)}>{running ? "Halt" : "Run"}</Button>
            <Button style={{ minWidth: 0 }} onClick={() => { setSelected(null); setIsolate(null); setSeed((s) => s + 1); }}>Reseed</Button>
          </div>
        </Panel>
        ...
        <Panel style={{ flexShrink: 0 }}>
          ...
          {NODE_CATEGORY_IDS.map((k) => (
            <ToggleRow
              ...
              icon={<Glyph shape={GLYPH_SHAPES[NODE_CATEGORIES[k]!.shape]} colour={NODE_CATEGORIES[k]!.color} muted={!nodeOn[k]} />}
              label={NODE_CATEGORIES[k]!.label}
              meta={NODE_CATEGORIES[k]!.tier === 0 ? "ALWAYS" : `z${TIER_ZOOM[NODE_CATEGORIES[k]!.tier]!.toFixed(1)}`}
            />
          ))}
          ...
          {LINK_CATEGORY_IDS.map((k) => (
            <ToggleRow
              ...
              icon={<LinkGlyph colour={LINK_CATEGORIES[k]!.color} dashed={!!LINK_CATEGORIES[k]!.dash} arrow={!!LINK_CATEGORIES[k]!.arrow} width={LINK_CATEGORIES[k]!.width * 1.05} muted={!linkOn[k]} />}
      ...
      <InspectorDrawer
        selected={selected}
        onClose={() => { setSelected(null); setIsolate(null); }}
        isolate={isolate}
        onIsolate={() => {
          if (!selected) return;
        ...
        onFocus={() => { if (selected) controllerRef.current?.focus(selected.id); }}
        onGoTo={goTo}
      />
...
function InspectorDrawer({
  selected, onClose, isolate, onIsolate, onFocus, onGoTo,
}: {
    ...
    >
      {selected && (
        <Panel padded={false} raised style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0 }}>
          <div style={{ padding: "var(--nx-space-5)", borderBottom: "var(--nx-hairline) solid var(--nx-border-default)", flexShrink: 0 }}>
            <div style={{ display: "flex", alignItems: "flex-start", gap: "var(--nx-space-3)" }}>
              <div style={{ paddingTop: 2 }}>
                <Glyph shape={GLYPH_SHAPES[NODE_CATEGORIES[selected.categoryId]!.shape]} colour={NODE_CATEGORIES[selected.categoryId]!.color} />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div id={titleId} style={{
                  color: NODE_CATEGORIES[selected.categoryId]!.color, fontFamily: "var(--nx-font-mono)", fontSize: "var(--nx-text-md)",
          ...
          <div style={{
            padding: "var(--nx-space-4) var(--nx-space-5)", display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))",
            ...
            <div>
              ...
              <div style={{ color: NODE_CATEGORIES[selected.categoryId]!.color, fontSize: "var(--nx-text-xs)", letterSpacing: "var(--nx-track-normal)" }}>{NODE_CATEGORIES[selected.categoryId]!.label}</div>
            </div>
          ...
          <div style={{ padding: "var(--nx-space-4) var(--nx-space-5) var(--nx-space-1)", flexShrink: 0 }}>
            ...
            {selected.groups.map((g) => (
              <MeterRow key={g.categoryId} label={LINK_CATEGORIES[g.categoryId]!.label} value={g.rows.length} total={selected.degree} colour={LINK_CATEGORIES[g.categoryId]!.color} />
            ))}
          </div>
          ...
          <div style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: "var(--nx-space-2) var(--nx-space-5) var(--nx-space-4)" }}>
            ...
            {selected.groups.map((g) => (
              <div key={g.categoryId} style={{ marginBottom: "var(--nx-space-3)" }}>
                <div style={{ color: LINK_CATEGORIES[g.categoryId]!.color, fontSize: "var(--nx-text-2xs)", letterSpacing: "var(--nx-track-wider)", opacity: 0.75, margin: "var(--nx-space-2) 0" }}>
                  {LINK_CATEGORIES[g.categoryId]!.label}
                </div>
                {g.rows.map((r) => (
                  <button key={String(r.id)} type="button" className="nx-row" onClick={() => onGoTo(r.id as number)}
                    style={{ width: "100%", background: "none", border: 0, textAlign: "left", paddingLeft: 0 }}>
                    <span aria-hidden="true" style={{ color: LINK_CATEGORIES[g.categoryId]!.color, width: 8, flexShrink: 0 }}>{r.out ? "▸" : "◂"}</span>
                    <Glyph shape={GLYPH_SHAPES[NODE_CATEGORIES[r.categoryId]!.shape]} colour={NODE_CATEGORIES[r.categoryId]!.color} size={10} />
                    <span style={{
                      flex: 1, color: NODE_CATEGORIES[r.categoryId]!.color, textTransform: "uppercase",
                      overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", letterSpacing: "var(--nx-track-tight)",
                    }}>{r.label}</span>
                    <span style={{ color: "var(--nx-fg-disabled)", fontSize: "var(--nx-text-2xs)", flexShrink: 0 }}>{NODE_CATEGORIES[r.categoryId]!.code}</span>
                  </button>
                ))}
              </div>
            ))}
