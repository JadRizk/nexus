    ...
    useEffect(() => {
      ...
      function boot(mountEl: HTMLDivElement, labelEl: HTMLDivElement) {
        ...
        function frame(now: number) {
          ...
          if (c.glitch > 0 && clock > glitchUntil && Math.random() < 0.0022 * c.glitch)
          ...
          if (mode !== "off") {
            ...
            for (let k = 0; k < cand.length && screenPos.size < POOL; k++) {
              ...
              for (let q = 0; q < boxes.length; q++) {
                const p = boxes[q]!;
                if (bx < p[0] + p[2] && bx + bw > p[0] && by < p[1] + LAB_H && by + LAB_H > p[1]) {
                  hit = true;
                  break;
                }
              }
          ...
          if (fT > 0.5) {
            const stats: GraphStats = {
              ...
              vertexAttribs: glCaps.attribs,
              webglVersion: glCaps.ver as 1 | 2,
            };
            onStats?.(stats);
            fA = 0;
            fN = 0;
